...
router.get('/', async (req, res) => {
  ...
  let q = supabase.from('rooms').select('*').order('room_number')
...
router.get('/:id', async (req, res) => {
  const { data, error } = await supabase.from('rooms').select('*, tenants(id,full_name,status)').eq('id', req.params.id).single()
